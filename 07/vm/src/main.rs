...
fn binary_op(op: &str) {
    ...
    clean_print(format!(r"@SP
    AM=M-1 // get and update SP at the same time
    D=M    // save y
    A=A-1
    M={}   // calculate x `op` y", op));
}
...
fn cmp_op(op: &str, line_no: usize) {
    ...
    (iftrue.{})", line_no, op, line_no));
}

fn push_virtual(symbol: &str, index: u16) {
...
fn push_ram(address: u16, index: u16) {
    clean_print(format!(r"@{}
                          D=M   // get value
                          @SP
                          M=M+1 // update SP
...
fn push_static(file: &str, index: u16) {
    clean_print(format!(r"@{}.{}
                          D=M   // get value
                          @SP
                          M=M+1 // update SP
...
fn pop_virtual(symbol: &str, index: u16) {
    clean_print(format!(r"@{}
                          D=M
                          @{}
                          D=D+A
...
fn pop_ram(address: u16, index: u16) {
    clean_print(format!(r"@SP
                          AM=M-1 // Decrement and seek to SP
                          D=M
                          @{}
...
fn pop_static(file: &str, index: u16) {
    clean_print(format!(r"@SP
                          AM=M-1 // Decrement and seek to SP
                          D=M
                          @{}.{}
...
fn pop_op(segment: &str, index: u16) {
    match segment {
